...
startp.addEventListener('focus', function () {
  this.select();
});
endp.addEventListener('focus', function () {
  this.select();
});
var length = 1; // 视频长度
// 初始化输入框的值 & progress
...
chrome.tabs.query({
  ...
  var url = tabs[0].url; // 获取url中的p参数，正则表达式：p=数字

  var p = url.match(/p=(\d+)/); // 获取p中的数字

  p = p ? parseInt(p[1]) : null; // 初始化输入框的值
  ...
  chrome.tabs.sendMessage(tabs[0].id, {
    ...
    var width = res[1];
    progress.style.width = "".concat(width, "%");
    progress.setAttribute('aria-valuenow', width);
    document.getElementById('prgs_info').innerText = "\u5F53\u524D\u8FDB\u5EA6...".concat(width, "%");
  })["catch"](function (err) {
    console.error(err);
  });
}); // 绑定range的input事件
...
if (ranger) {
  ranger.addEventListener('input', function () {
    endp.value = parseInt(this.value); // ranger的颜色随着值的变化而变化
...
if (btn_cal) {
  btn_cal.addEventListener('click', function () {
    ...
    var p2 = parseInt(endp.value); // 如果p1>p2,则交换p1和p2的值
    ...
    chrome.tabs.query({
      ...
      chrome.tabs.sendMessage(tabs[0].id, message).then(function (res) {
        // alert("sendMessage!")
        stime.value = "".concat(res[0], "h ").concat(res[1], "min ").concat(res[2], "s");
      })["catch"](function (err) {
        console.error(err);
      });
    });
